...
func GetPostsByMyPost(userID int) ([]Post, error) {
	...
	query := `
		SELECT p.id, p.title, p.content, p.user_id, p.category, 
	...
	for rows.Next() {
		var post Post
		err := rows.Scan(&post.ID, &post.Title, &post.Content, &post.UserID, &post.Category, &post.LikeCount, &post.DislikeCount, &post.Username)
		if err != nil {
			return nil, err
		}
